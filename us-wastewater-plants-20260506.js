(function () {
  ...
  function resetFilters() {
    ...
    var lockedState = container ? (container.getAttribute('data-state') || '').toUpperCase() : '';
    $('#' + SLUG + '-search').value = '';
    $('#' + SLUG + '-state').value = (lockedState && STATE_NAMES[lockedState]) ? lockedState : '';
    var allRadio = document.getElementById(SLUG + '-src-all');
    if (allRadio) allRadio.checked = true;
    applyFilters();
    if (!lockedState) map.setView([39.5, -98.5], 4);
  }
  ...
  function init() {
    ...
    if (lockedState && STATE_NAMES[lockedState]) {
      var stateSel = $('#' + SLUG + '-state');
      stateSel.value = lockedState;
      stateSel.style.display = 'none';
      // Hide the "Boundary source" group label too — keep just the radios for compactness
      var stateLabel = container.querySelector('.uwp-toolbar .uwp-group-label');
      // (leave the source radio group visible — useful filter on state pages)
    }
    ...
    fetchPolygonAttrs()
      ...
      .then(function () {
        ...
        if (saved) {
          // Hash overrides data-state when present (lets users share a different state from a spoke page)
        ...
        } else if (lockedState && STATE_NAMES[lockedState]) {
          $('#' + SLUG + '-state').value = lockedState;
        }
        applyFilters();
