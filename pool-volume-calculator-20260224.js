(function () {
  ...
  var flatDepthM     = 1.52;   // ~5.0 ft default
  var shallowDepthM  = 1.07;   // ~3.5 ft default
  var deepDepthM     = 1.83;   // ~6.0 ft default
  var depthMode      = 'flat'; // 'flat' | 'sloped'
  var units          = 'imperial';
      ...
      '<div class="pvc-results">',
        ...
        '<div class="pvc-results-grid">',
          ...
          '<div class="pvc-depth-card">',
            ...
            // Flat / sloped toggle
            '<div class="pvc-depth-mode-row">',
              '<button class="pvc-depth-mode-btn pvc-mode-active" id="pvc-mode-flat">Flat Bottom</button>',
              '<button class="pvc-depth-mode-btn" id="pvc-mode-sloped">Shallow + Deep</button>',
            '</div>',
            // Flat bottom input
            '<div class="pvc-depth-fields" id="pvc-flat-row">',
              '<div class="pvc-depth-field">',
                '<label for="pvc-flat-depth" id="pvc-flat-label">Pool depth (ft)</label>',
                '<input type="number" id="pvc-flat-depth" class="pvc-depth-input" min="0.1" max="30" step="0.5" value="5.0" />',
              '</div>',
            '</div>',
            // Sloped inputs
            '<div class="pvc-depth-fields" id="pvc-sloped-row" style="display:none;">',
              '<div class="pvc-depth-field">',
  ...
  function setDepthMode(mode) {
    depthMode = mode;
    el('pvc-flat-row').style.display   = mode === 'flat'   ? '' : 'none';
    el('pvc-sloped-row').style.display = mode === 'sloped' ? '' : 'none';
    el('pvc-mode-flat').classList.toggle('pvc-mode-active',   mode === 'flat');
    el('pvc-mode-sloped').classList.toggle('pvc-mode-active', mode === 'sloped');
    if (currentPolygon) calculate();
  }
  ...
  function syncDepthInputLabels() {
    ...
    if (el('pvc-flat-label'))    el('pvc-flat-label').textContent    = 'Pool depth (' + unit + ')';
    if (el('pvc-shallow-label')) el('pvc-shallow-label').textContent = 'Shallow end (' + unit + ')';
    if (el('pvc-deep-label'))    el('pvc-deep-label').textContent    = 'Deep end (' + unit + ')';
  }

  function syncDepthInputValues() {
    var fi = el('pvc-flat-depth');
    var si = el('pvc-shallow-depth');
    var di = el('pvc-deep-depth');
    if (units === 'metric') {
      if (fi) { fi.step = '0.1'; fi.value = flatDepthM.toFixed(2); }
      if (si) { si.step = '0.1'; si.value = shallowDepthM.toFixed(2); }
      if (di) { di.step = '0.1'; di.value = deepDepthM.toFixed(2); }
    } else {
      if (fi) { fi.step = '0.5'; fi.value = toFt(flatDepthM).toFixed(1); }
      if (si) { si.step = '0.5'; si.value = toFt(shallowDepthM).toFixed(1); }
      if (di) { di.step = '0.5'; di.value = toFt(deepDepthM).toFixed(1); }
    }
  }
  ...
  function calculate() {
    ...
    var avgDepthM;
    if (depthMode === 'flat') {
      avgDepthM = flatDepthM;
    } else {
      avgDepthM = (shallowDepthM + deepDepthM) / 2;
    }
  ...
  function showResults(areaSqM, areaSqFt, avgDepthM, avgDepthFt, gallons, litres) {
    ...
    if (units === 'metric') {
      ...
      el('pvc-avgdepth-sub').textContent   = fmt(avgDepthFt, 1) + ' ft';
      ...
      el('pvc-avgdepth-sub').textContent   = fmt(avgDepthM, 2) + ' m';
  ...
  function bindEvents() {
    ...
    // Depth mode toggle
    el('pvc-mode-flat').addEventListener('click', function () { setDepthMode('flat'); });
    el('pvc-mode-sloped').addEventListener('click', function () { setDepthMode('sloped'); });

    // Depth inputs
    el('pvc-flat-depth').addEventListener('input', function () {
      var v = parseFloat(this.value);
      if (!isNaN(v) && v > 0) { flatDepthM = units === 'metric' ? v : toM(v); calculate(); }
    });
  ...
  function loadStateFromUrl() {
    ...
    if (params.depthMode === 'flat' || params.depthMode === 'sloped') depthMode = params.depthMode;
    if (params.flatM)    flatDepthM    = parseFloat(params.flatM);
    ...
    if (params.poly) {
      try {
        ...
        var poly    = L.polygon(latlngs, {
          color: CONFIG.poolColor, fillColor: CONFIG.poolColor, fillOpacity: 0.25, weight: 2,
        });
        drawnItems.clearLayers();
        currentPolygon = poly;
