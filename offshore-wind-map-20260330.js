(function () {
  ...
  function buildWhere() {
    ...
    return "label='wind' AND label_confidence IN (" +
  ...
  function buildPopup(props) {
    ...
    var color = CONF_COLORS[conf] || '#64b5f6';
    ...
    return '<div style="font-family:-apple-system,sans-serif;min-width:195px;font-size:0.82rem;">' +
      '<div style="font-weight:700;font-size:0.88rem;color:#0d47a1;margin-bottom:7px;padding-bottom:5px;border-bottom:2px solid ' + color + ';">Offshore Wind Structure</div>' +
  ...
  function updateCount() {
    ...
    fetch(url)
      ...
      .then(function (d) {
        var el = document.getElementById('owm-count');
        if (el && d.count !== undefined) {
          el.textContent = d.count.toLocaleString() + ' structures';
        }
      })
      .catch(function () {});
  }
  ...
  function initMap() {
    ...
    var lat = state.lat ? parseFloat(state.lat) : 40;
    var lng = state.lng ? parseFloat(state.lng) : 10;
    ...
    map = L.map('offshore-wind-map-map', { center: [lat, lng], zoom: z });
  ...
  function syncCheckboxes() {
    ['high', 'medium', 'low'].forEach(function (k) {
      var cb = document.getElementById('owm-cb-' + k);
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var shareBtn = document.getElementById('owm-share-btn');
    if (shareBtn) {
