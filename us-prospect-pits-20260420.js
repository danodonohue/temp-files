(function () {
  ...
  var CONFIG = {
    slug: 'us-prospect-pits',
    typeName: 'ms:points',
    filterXml: '<Filter><PropertyIsEqualTo><PropertyName>ftr_type</PropertyName><Literal>Prospect Pit</Literal></PropertyIsEqualTo></Filter>',
    markerColor: '#D4A017',
    pageSize: 500,
  ...
  function updateShareUrl() {
    ...
    var hash = '#lat=' + c.lat.toFixed(4) + '&lng=' + c.lng.toFixed(4) + '&z=' + map.getZoom() +
      (selectedState ? '&state=' + encodeURIComponent(selectedState) : '') +
      (activeBasemap === satelliteLayer ? '&layer=sat' : '');
  ...
  function getEl(parent, name) {
    var el = parent.getElementsByTagNameNS('*', name)[0];
    return el ? el.textContent.trim() : '';
  }

  function setStatus(msg) {
    var el = document.getElementById(CONFIG.slug + '-status');
  ...
  function parseGML(xmlText) {
    ...
    for (var i = 0; i < members.length; i++) {
      ...
      features.push({
        lat: lat, lng: lng,
        state: getEl(m, 'state'), county: getEl(m, 'county'),
        ftr_type: getEl(m, 'ftr_type'), ftr_name: getEl(m, 'ftr_name'),
        topo_name: getEl(m, 'topo_name'), topo_date: getEl(m, 'topo_date'),
        url: getEl(m, 'url')
      });
    }
    return features;
  ...
  function fetchPage(startIndex) {
    var url = WFS_BASE + '?service=WFS&version=2.0.0&request=GetFeature' +
      '&typeName=' + CONFIG.typeName + '&count=' + CONFIG.pageSize +
      '&startIndex=' + startIndex + '&FILTER=' + encodeURIComponent(CONFIG.filterXml);
    fetch(url).then(function (r) { return r.text(); }).then(function (xml) {
      ...
      if (features.length === CONFIG.pageSize) {
        fetchPage(startIndex + features.length);
      } else {
        setStatus('');
        populateStateFilter();
      }
  ...
  function addFeature(f) {
    ...
    var marker = L.circleMarker([f.lat, f.lng], {
      radius: 5, fillColor: CONFIG.markerColor,
    ...
    turfFC.features.push({
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [f.lng, f.lat] },
      properties: { ftr_type: f.ftr_type, ftr_name: f.ftr_name, state: f.state, county: f.county, topo_name: f.topo_name, topo_date: f.topo_date, url: f.url }
    });
  }
  ...
  function downloadGeoJSON() {
    ...
    var blob = new Blob([JSON.stringify({
      ...
      features: list.map(function (m) {
        var f = m.data;
        return { type: 'Feature', geometry: { type: 'Point', coordinates: [f.lng, f.lat] }, properties: { ftr_type: f.ftr_type, ftr_name: f.ftr_name, state: f.state, county: f.county, topo_name: f.topo_name, topo_date: f.topo_date } };
      })
    })], { type: 'application/json' });
    var a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = CONFIG.slug + (selectedState ? '-' + selectedState : '') + '.geojson';
  ...
  function init() {
    ...
    streetsLayer = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors | Data: <a href="https://mrdata.usgs.gov/">USGS Mineral Resources</a>',
      maxZoom: 19
    });
    satelliteLayer = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
      attribution: 'Tiles &copy; Esri | Data: <a href="https://mrdata.usgs.gov/">USGS Mineral Resources</a>',
      maxZoom: 19
    });
    ...
    map.on('click', function (e) {
      if (_suppressNearest) { _suppressNearest = false; return; }
      nearestPopup(e);
    });
    ...
    if (toggleBtn) {
      ...
      toggleBtn.addEventListener('click', function () {
        ...
        activeBasemap = (activeBasemap === streetsLayer) ? satelliteLayer : streetsLayer;
        activeBasemap.addTo(map);
        this.textContent = (activeBasemap === streetsLayer) ? 'Satellite' : 'Street Map';
        updateShareUrl();
      });
    }
