...
(function() {
    ...
    function createPopupContent(props) {
        ...
        if (props.Shape__Area) {
            ...
            area = `${sqMiles} sq mi (${acres.toLocaleString()} acres)`;
        }
    ...
    function showInfoPanel(props) {
        ...
        if (props.Shape__Area) {
            ...
            const sqMiles = (sqMeters / 2589988.11).toFixed(1);
            const acres = (sqMeters / 4046.86).toFixed(0);
            areaHtml = `<p><strong>Area:</strong> ${sqMiles} square miles<br/>(${acres.toLocaleString()} acres)</p>`;
        }

        let perimeterHtml = '';
        if (props.Shape__Length) {
            const meters = props.Shape__Length;
            const miles = (meters / 1609.34).toFixed(1);
            perimeterHtml = `<p><strong>Perimeter:</strong> ${miles} miles</p>`;
        }
        ...
        panel.innerHTML = `
            ...
            <p><strong>Province:</strong> ${province}</p>
            <p><strong>Geological Age:</strong> ${age}</p>
            <p><strong>Coal Rank:</strong> ${rank}</p>
            ${areaHtml}
            ${perimeterHtml}
            <p class="ucf-hint">Click another field to view its details</p>
        `;
    ...
    function showInitialHint() {
        ...
        panel.className = 'ucf-info-panel';
        panel.innerHTML = `
            <button class="ucf-close-btn" onclick="this.parentElement.remove()">&times;</button>
            <h3>USA Coal Fields</h3>
            <p>Click any coal field to view detailed information about its geology, rank, and extent.</p>
            <p class="ucf-hint">Each color represents a different coal field or region</p>
        `;
        document.getElementById('ucf-container').appendChild(panel);
    }
